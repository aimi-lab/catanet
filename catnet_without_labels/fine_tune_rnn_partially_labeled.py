...
from utils.dataset_utils import DatasetPartiallyLabeled
...
def masked_loss(criterion, prediction, target, mask):
...
def main(output_folder, log, pretrained_model, config):
    ...
    model.load_state_dict(torch.load(config['pretrained_model'])['model_dict'])
    model.set_cnn_as_feature_extractor()
        ...
        if training_step == 'train_rnn':
            ...
            if len(features) == 0:
                ...
                for ii, input_path in enumerate(sequences):
                    print(input_path)
                    label = glob.glob(os.path.join(input_path, '*.csv'))
                    data = DatasetPartiallyLabeled([input_path], label, img_transform=img_transform)
                    dataloader = DataLoader(data, batch_size=500, shuffle=False, num_workers=1, pin_memory=True)
                    features[input_path] = []
                    rsd_labels[input_path] = []
                    step_labels[input_path] = []
                    exp_labels[input_path] = []
                    for i, (X, _, _, y_rsd, y_step, y_exp) in enumerate(dataloader):
                        with torch.no_grad():
                            features[input_path].append(model.cnn(X.float().to(device)).cpu().numpy())
                            rsd_labels[input_path].append(y_rsd.numpy())
                            step_labels[input_path].append(y_step.numpy())
                            exp_labels[input_path].append(y_exp.numpy())
                    features[input_path] = np.concatenate(features[input_path])
                    rsd_labels[input_path] = np.concatenate(rsd_labels[input_path])
                    step_labels[input_path] = np.concatenate(step_labels[input_path])
                    exp_labels[input_path] = np.concatenate(exp_labels[input_path])
                ...
                for ii,  input_path in enumerate(sequences_path[phase]['video']):
                    if (training_step == 'train_rnn') | (training_step == 'train_fc'):
                        dataloader = [(torch.tensor(features[input_path]).unsqueeze(0),
                                       ...
                                       torch.tensor(step_labels[input_path]).unsqueeze(0),
                                       torch.tensor(exp_labels[input_path]).unsqueeze(0))]
                        skip_features = True
                    else:
                    ...
                    for i, (X, _, _, y_rsd, y_step, y_exp) in enumerate(dataloader):
                        if len(y_rsd.shape) == 2:
                            ...
                            y_step = y_step.squeeze(0)
                            y_exp = y_exp.squeeze(0)
                        y_rsd = y_rsd.float().to(device)
                        y_step = y_step.long().to(device)
                        y_exp = y_exp.long().to(device)
                        X = X.float().to(device)
                        label_mask = (y_exp >= 0)

                        with torch.set_grad_enabled(phase == 'train'):
                            stateful = (i > 0)
                            step_prediction, experience_prediction, rsd_prediction = model.forwardRNN(X, stateful=stateful, skip_features=skip_features)
                            loss_step = masked_loss(step_criterion, step_prediction, y_step, label_mask)
                            loss_experience = masked_loss(experience_criterion, experience_prediction, y_exp, label_mask)
                            loss_rsd = rsd_criterion(rsd_prediction.squeeze(1), y_rsd)
